...
- 规则页新增 `新建规则`，支持手动填写包名、可选 Activity、功能和允许/拒绝结果。
- 已有规则支持点击编辑；规则列表现在显示应用名、包名、Activity 范围、功能和当前策略，Activity 为空时表示整包规则。
- 系统侧策略匹配支持 Activity 精确规则优先、整包规则次之、全局默认策略兜底。
- 最近事件中包名和 Activity 支持长按复制，并用 Toast 提示复制结果。
- 最近事件连续分组的时间范围改为 `MM-dd HH:mm:ss -- HH:mm:ss`；跨天时结束时间显示月日。
